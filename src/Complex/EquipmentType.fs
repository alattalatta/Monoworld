...
type EquipmentType =
    ...
    override this.BuildRequirementString() =
        let apparel =
            if this.apparel then Some L18N.Complex.apparel else None

        let melee =
            if this.melee then Some L18N.Complex.melee else None

        let ranged =
            if this.ranged then Some L18N.Complex.ranged else None
